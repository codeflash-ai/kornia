...
def get_projective_transform(center: Tensor, angles: Tensor, scales: Tensor) -> Tensor:
    ...
    Args:
        ...
        angles: axis angle vector containing the rotation angles in degrees in the form
            of (rx, ry, rz) with shape :math:`(B, 3)`.
    ...
    if center.device != angles.device or center.dtype != angles.dtype:
        raise AssertionError(center.device, angles.device)

    # Compute rotation matrix
    axis_angle_rad = deg2rad(angles)
    rmat = axis_angle_to_rotation_matrix(axis_angle_rad)  # Bx3x3

    # Compute scaling matrix and apply scaling
    scales = scales.unsqueeze(1)
    rmat *= scales.to(rmat)

    # Construct translation matrices to handle rotation around a center
    ...
    proj_mat = torch.cat([rmat, torch.zeros((batch_size, 3, 1), dtype=rmat.dtype, device=rmat.device)], dim=2)
    proj_mat_4x4 = convert_affinematrix_to_homography3d(proj_mat)

    # Combine transformations
    proj_mat = from_origin_mat @ proj_mat_4x4 @ to_origin_mat

    # Return as (B, 3, 4) projection matrix
    return proj_mat[..., :3, :]
